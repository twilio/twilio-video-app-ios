...
//            context("when a participant on page 2 of 3 becomes dominant speaker") {
//                beforeEach {
//                    Array(1...12).forEach { sut.addParticipant(.stub(identity: String($0), dominantSpeakerStartTime: Date())) }
//                    sut.pages[0].participants[2].dominantSpeakerStartTime = .distantPast
//
//                    sut.updateParticipant(.stub(identity: "9", isDominantSpeaker: true, dominantSpeakerStartTime: Date()))
//                }
//                
//                it("is moved to the position of the oldest dominant speaker on the first page") {
//                    expect(sut.pages[0].participants.map { $0.identity }).to(equal(["1", "2", "9", "4", "5", "6"]))
//                }
//                
//                it("moves the oldest dominant speaker from the first page to the beginning of the second pages") {
//                    expect(sut.pages[1].participants.map { $0.identity }).to(equal(["3", "7", "8", "10", "11", "12"]))
//                }
//            }

            context("when a participant on page 3 becomes dominant speaker") {
                beforeEach {
            ...
            // Creates a new page
            context("when only 1 user on second page") {
                beforeEach {
            ...
            // Last participant on last page was removed
            context("when only 1 user on third page") {
                beforeEach {
