...
//        [self.sampleView removeObserver:self forKeyPath:@"layer.status"];
    ...
    override func didMoveToSuperview() {
        ...
        print("TCR constraints: \(constraints)")
    }
}
...
//            return
//        } else
        if (pixelFormat == PixelFormat.formatYUV420PlanarFullRange.rawValue ||
...
//        }
        DispatchQueue.main.sync {
            _ = self.detectFormatChange(imageBuffer: frame.imageBuffer)
            self.enqueueFrame(frame: frame)
        }
    }
